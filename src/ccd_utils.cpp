...
void ccd_utils::sumColumnsByGroup(const std::vector<double>& matrix, size_t rows, size_t cols, const std::vector<size_t>& membership, std::vector<double>& result) {
    // Determine the number of unique groups
    unordered_map<size_t, size_t> groupIndex;
    size_t groupCount = 0;
    for (size_t group : membership) {
